...
def main(args):
    ...
    head_importance["encoder_self"] += torch.from_numpy(
        importances[:encoder_layers, 1:]).float()
    head_importance["encoder_decoder"] += torch.from_numpy(
        importances[encoder_layers:encoder_layers+decoder_layers, 1:]).float()
    head_importance["decoder_self"] += torch.from_numpy(
        importances[encoder_layers+decoder_layers:, 1:]).float()
